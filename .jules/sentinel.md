...
## 2025-02-15 - PostgreSQL Column Level IDOR via Trigger Omission
**Vulnerability:** Regular users could arbitrarily update their family's `verification_status` and their own `join_request` `status` because while Row Level Security (RLS) allowed them to update the *row*, there were no restrictions preventing them from modifying these specific sensitive columns.
**Learning:** RLS `UPDATE` policies apply to the entire row. If a user is authorized to update *any* field in a row (like their name or family name), they are implicitly authorized to update *all* fields unless column-level restrictions are in place. This allows IDOR mass-assignment privilege escalations if sensitive columns like `status` or `role` exist in the same table.
**Prevention:** Use PostgreSQL `BEFORE UPDATE` triggers (or column-level `REVOKE`) to explicitly block unauthorized state transitions on sensitive columns, ensuring they bypass service roles so backend processes aren't blocked.
