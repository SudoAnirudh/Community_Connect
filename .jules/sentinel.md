...
## 2024-06-29 - [PostgreSQL Infinite Recursion in RLS UPDATE Policies]
**Vulnerability:** The RLS `UPDATE` policy for the `users` table utilized a self-referencing subquery (`select role from users where uid = public.auth_uid_text()`) inside the `WITH CHECK` clause to prevent unauthorized modifications to sensitive columns (like `role` or `suspended`).
**Learning:** Using self-referencing subqueries in RLS `WITH CHECK` conditions on the same table often causes severe performance degradation and triggers `ERROR: infinite recursion detected` during write operations.
**Prevention:** Avoid self-referencing subqueries inside RLS `WITH CHECK` clauses. To restrict updates on specific sensitive columns safely, utilize `BEFORE UPDATE` triggers (with `SECURITY DEFINER` and `SET search_path = public`) to handle column-level authorization checks.
