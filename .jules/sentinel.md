...
\n## 2024-07-28 - [Event Status Privilege Escalation Fixed]\n**Vulnerability:** The RLS INSERT policy for the events table did not restrict the status column, allowing any user to set a custom status (e.g. 'approved') during event creation.\n**Learning:** Default values in the schema only apply if the field is omitted in the API payload. If included, they bypass the default unless explicitly blocked by WITH CHECK.\n**Prevention:** Added `status = 'upcoming'` to the WITH CHECK clause on the events INSERT policy to force all user-created events to start in a safe initial state.
