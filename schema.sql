...
-- Trigger to protect user role and status
...
begin
  ...
  if old.role is distinct from new.role or old.suspended is distinct from new.suspended or old.uid is distinct from new.uid then
    -- Only allow system admins to change role, suspended status, or uid
    if not exists (
      select 1 from users u where u.uid = public.auth_uid_text() and u.role = 'admin'
    ) then
      raise exception 'Unauthorized to update role, suspended status, or uid';
    end if;
  end if;
